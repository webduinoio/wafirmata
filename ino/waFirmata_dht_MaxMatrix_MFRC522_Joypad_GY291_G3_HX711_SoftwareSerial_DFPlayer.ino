    ...
    case 4:
      switch (argv[0]) {
        ...
        case 0x16:  //SoftwareSerial data exchange
          ...
          break;
        case 0x19:  //MP3 DFPlayer mini
          switch (argv[1]) { //argv[1] = cmd;
            case 0: //init
              if (dfSerial == NULL) {
                dfSerial = new SoftwareSerial(argv[2] /*RX*/, argv[3] /*TX*/);
                dfSerial->begin(9600);
                dfPlayer = new DFRobotDFPlayerMini();
                if (!dfPlayer->begin((SoftwareSerial&)*dfSerial)) {
                  Firmata.sendString("mp3 init failure!");
                } else {
                  Firmata.sendString("mp3 init OK");
                  respSysex(0x19, 0);
                }
              } else {
                respSysex(0x19, 0);
...
void respSysex(byte sensorType, byte code) {
  Firmata.write(START_SYSEX);
  Firmata.write(4);
  Firmata.write(sensorType);
  Firmata.write(code);
  Firmata.write(END_SYSEX);
}
