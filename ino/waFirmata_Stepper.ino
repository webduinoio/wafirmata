...
void setPinModeCallback(byte pin, int mode)
{
  ...
  switch (mode) {
    ...
    case I2C:
      ...
      break;
    default:
      Firmata.sendString(("Unknown pin Mode:(" + String(mode) + ")").c_str()); // TODO: put error msgs in EEPROM
  }
...
{
  byte pin, lastPin, mask = 1, pinWriteMask = 0, nowState;
    ...
    for (pin = port * 8; pin < lastPin; pin++) {
      ...
      if (IS_PIN_DIGITAL(pin)) {
        ...
        if (pinConfig[pin] == OUTPUT || pinConfig[pin] == INPUT) {
          pinWriteMask |= mask;
          nowState =  ((byte)value & mask) ? 1 : 0;
          if (pinState[pin] != nowState) {
            pinState[pin] = nowState;
            Firmata.sendDigitalPort(port, nowState);
            Firmata.sendString("test ok");
          }
        }
      }
      mask = mask << 1;
